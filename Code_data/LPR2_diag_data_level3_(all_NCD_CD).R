...
get_icd10_diagnosis_data <- function(icd10_codes, pop_id, only_A_diag = FALSE){
  ...
  for(i in 1:n_batches){
    ...
    diags[[i]] <- trawlDiagnosisData(
      ...
      mutate(diag = substr(str_trim(diag), 1, 4)) %>% 
      get_first_diag(.) %>% 
      filter(diag %in% icd10_codes)
    
  }
  diags <- bind_rows(diags) %>% dplyr::select(person_id, diag, d_inddto, d_uddto, source)
  t1 <- Sys.time()
...
get_icd8_diagnosis_data <- function(icd10_codes, icd_map, pop_id, only_A_diag = FALSE){
  ...
  diags_icd8 <- diags_icd8 %>% 
    ...
    filter(diag %in% icd10_codes) %>% 
    get_first_diag(.) %>% 
    dplyr::select(person_id, diag, d_inddto, d_uddto, source)
...
LL <- dbGetQuery(conn_LL, ("select DISTINCT PERSON_ID from D221911.LIFELINES"))
...
icd10_codes <- bind_rows(icd10_codes_lpr, icd10_codes_priv, icd10_codes_uaf, icd10_codes_psyk) %>% 
  mutate(DIAG = str_sub(str_trim(ICD10_CODE), 1, 4)) %>% 
  distinct(DIAG) %>%
  # Only need ICD10 chapters 1-19 which ends with DT98
  filter(DIAG <= 'DT98') %>% 
...
DBI::dbWriteTable(conn_inci, 'diags_LPR2_level3', diags, overwrite = TRUE)
