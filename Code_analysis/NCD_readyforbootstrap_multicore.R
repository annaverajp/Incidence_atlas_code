...
diags_NCD <- dbReadTable(conn = conn_inci, schema= "D222107",  name = "diags_NCD")
...
inference <- function(Population,bootstrapindices,diags,bootEDU=FALSE,bootINC= FALSE, return_names=FALSE){
  ...
  incidence <- left_join(tibble(NCD = incidence$NCD,ID = seq.int(length(incidence$NCD))), tibble(incidence$result, ID = seq.int(nrow(incidence$result)))) %>%
    dplyr::select(-c(ID)) 
...
#setup new cluster
max_threads_dt = 15 #ensure maximum 16 cores, including subthreads by workers.
...
original_estimates <- inference(Population = Population,bootstrapindices = 1:nrow(Population), diags = diags_NCD,return_names = TRUE)

write_csv2(original_estimates, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/original_estimates_diags_NCDonly.csv"))
...
#     
#     ###### ------------- Clean data ------------------------###########
#     diags_NCD <-diags_NCD %>% mutate(D_INDDTO = cal.yr(D_INDDTO))
...
#     
#     write_csv2(booted_all, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/booted_all_i",i,"_diags_NCDonly.csv"))
...
#     #write to csvfile here, every minirun..
#     
#     write_csv2(booted_edu, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/booted_edu_i",i,"_diags_NCDonly.csv"))
#     print(Sys.time()-start) 
#   }
# ) 
# suppressWarnings(
...
#     #write to csvfile here, every minirun..
#     
#     write_csv2(booted_inc, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/booted_inc_i",i,"_diags_NCDonly.csv"))
#     print(Sys.time()-start) 
#   }
# )
