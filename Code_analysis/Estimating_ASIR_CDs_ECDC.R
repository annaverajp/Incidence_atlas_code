...
##############################################################################
# The following script calculates age standardized incidence rates for       #
# communicable diseases. CD are defined according to the definitions used in #
...
diags_CD_ECDC <- dbReadTable(conn = conn_inci, name = "diags_CD_ECDC")
...
ASIR_CD <- data.frame(ASIR_CD) %>% 
  mutate(udd = result$udd, 
         ASIR = result$ASIR) 

ASIR_CD <- within(ASIR_CD, rm(result))


dbWriteTable(conn_inci, "ASIR_CD_ECDC", ASIR_CD, overwrite = TRUE) 
