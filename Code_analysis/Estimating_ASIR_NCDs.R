...
##############################################################################
# The following script calculates age standardized incidence rates for non-  #
...
diags_NCD <- dbReadTable(conn = conn_inci, name = "diags_NCD")
...
ASIR_NCD <- data.frame(ASIR_NCD) %>% 
  mutate(udd = result$udd, 
         ASIR = result$ASIR) 

ASIR_NCD <- within(ASIR_NCD, rm(result))


dbWriteTable(conn_inci, "ASIR_NCD", ASIR_NCD, overwrite = TRUE) 
