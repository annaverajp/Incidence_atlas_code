...
diags_CD <- dbReadTable(conn = conn_inci, schema= "D222107",  name = "diags_CD")
...
inference <- function(Population,bootstrapindices,diags,bootEDU=FALSE,bootINC= FALSE, return_names=FALSE){
  ...
  get_incidence_rate <- function(sample, Population) {
    ...
    NA_tibble_returner <- function(isUDD=TRUE,KOEN=1.5, analysis="Education"){
      ...
      if(isUDD){
        ...
        NA_tibble <- expand_grid(INCOMEQ=1:4, KOEN={{KOEN}}) %>% 
          ...
          mutate(ASIR = NA, 
                 RR_type = case_when(SEP=="Q1"~"RR1v4",SEP=="Q2"~"RR2vs4",SEP=="Q3" ~"RR3vs4",SEP=="Q4"~"reference"), 
                 RR=NA, analysis={{analysis}}) %>% 
          relocate(SEP, .after = last_col())
      }
    ...
    if(udd_big_enough_for_reg){
      ...
      results_udd <- rbind(grid_data_udd1, grid_data_udd2, grid_data_udd3) %>% 
        ...
        mutate(RR1vs3 = log(udd1)-log(udd3),
               RR2vs3 = log(udd2)-log(udd3)) %>% 
    ...
    if(inc_big_enough_for_reg){ 
      ...
      results_income <- rbind(grid_data_Q1, grid_data_Q2, grid_data_Q3, grid_data_Q4) %>% 
        ...
        mutate(RR1vs4 = log(Q1)-log(Q4),
               RR2vs4 = log(Q2)-log(Q4),
               RR3vs4 = log(Q3)-log(Q4)) %>% 
    ...
    results <- rbind(results %>% mutate(estimator=str_c("log_RR_",RR_type), value=(as.numeric(RR))),
  ...
  incidence <- left_join(tibble(CD = incidence$CD,ID = seq.int(length(incidence$CD))), tibble(incidence$result, ID = seq.int(nrow(incidence$result)))) %>%
    dplyr::select(-c(ID)) 
...
original_estimates <- inference(Population = Population,bootstrapindices = 1:nrow(Population), diags = diags_CD,return_names = TRUE)

write_csv2(original_estimates, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/LOGTRANSFORM/original_estimates_diags_CDonly.csv"))
gc()

max_threads_dt = 2 #ensure maximum 16 cores, including subthreads by workers.
...
setupCluster <- function(){
  ...
  clusterEvalQ(cl, {
    ...
    ###### ------------- Clean data ------------------------###########
    diags_CD <-diags_CD %>% mutate(D_INDDTO = cal.yr(D_INDDTO))
...
suppressWarnings(
  for(i in 1:(60/numcores)){
    ...
    write_csv2(booted_all, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/LOGTRANSFORM/60booted_all_i",i,"_diags_CDonly.csv"))
...
suppressWarnings(
  for(i in 1:(30/numcores)){
    ...
    write_csv2(booted_edu, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/LOGTRANSFORM/booted_edu_i",i,"_diags_CDonly.csv"))
    print(Sys.time()-start)
  }
)
suppressWarnings(
  for(i in 1:(30/numcores)){
    ...
    write_csv2(booted_inc, paste0( "~/kdrev/Metode/Challenge/7_FKC/age_cohort/LOGTRANSFORM/booted_inc_i",i,"_diags_CDonly.csv"))
    print(Sys.time()-start)
  }
)
